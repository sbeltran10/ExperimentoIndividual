...
public class MaquinaEstructura {
	...
	public class ArbolBinario implements Operaciones{
		...
		private void inorden(NodoArbolBinario x, String msg) {

			if (x != null)
			{
				inorden(x.izquierda, msg);
				
				msg += "["+x.getId()+"]";

				inorden(x.derecha, msg);
			}
		}
		...
		{
			System.out.println(x.getId() + " //" + agregado.getId());
	...
	public static void main(String args[]){
		...
		MaquinaEstructura x = new MaquinaEstructura();
		
		ArbolBinario arbol = x.new ArbolBinario(x.new NodoArbolBinario(2));
		
		arbol.agregar(x.new Nodo(2));
		arbol.agregar(x.new Nodo(3));
		arbol.agregar(x.new Nodo(4));
